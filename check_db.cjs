...
async function check() {
  const { data: jobs, error: jobErr } = await supabase.from('jobs').select('id, job_code').order('created_at', { ascending: false }).limit(3);
  ...
  for (const job of jobs) {
    ...
    if (svcErr) {
      ...
      console.log(`Services for ${job.job_code} (${job.id}):`, svcs);
    }
  }
}
