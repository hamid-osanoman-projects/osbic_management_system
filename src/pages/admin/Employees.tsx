...
const Employees = () => {
  ...
  const handleConfirmReset = () => {
    if (selectedEmployee?.email) {
      resetPassword(selectedEmployee.email, {
        onSuccess: () => {
               ...
               <div className="flex justify-between items-start mb-4">
                 <div className="flex items-center gap-3">
                   <div className="relative">
                      ...
                      <div className={cn("absolute -bottom-1 -right-1 w-3.5 h-3.5 border-2 border-card rounded-full", emp.is_active !== false ? 'bg-emerald-500' : 'bg-red-500')} />
                   </div>
                   <div>
              ...
              <tbody>
                {filteredEmployees?.map((emp: any) => (
                  <tr key={emp.id} className="border-b border-border hover:bg-muted/30 transition-colors group">
                    ...
                    <td className="py-4 px-6 text-xs text-foreground">{emp.email}</td>
