      ...
      {isLoading ? (
        ...
        >
          {filteredServices?.map((service) => {
            ...
            return (
              <motion.div key={service.id} variants={itemAnimations} className="bg-card border border-border rounded-2xl p-6 shadow-sm group relative overflow-hidden flex flex-col">
                 ...
                 >
                   <div className="flex items-center gap-2 mb-1">
                     <h3 className="text-lg font-bold text-foreground group-hover/title:text-primary transition-colors truncate">{service.name_en}</h3>
                   ...
                   <p className="text-sm text-muted-foreground/60 font-medium" dir="rtl">{service.name_ar}</p>
                 </div>

                 <div className="flex flex-wrap items-center gap-2 mb-6 mt-auto">
                    <span className={cn(
                      "px-2 py-1 rounded-md text-[10px] lowercase font-bold tracking-widest border",
                      CATEGORY_COLORS[service.category] || CATEGORY_COLORS.other
                    )}>
                      {CATEGORY_LABELS[service.category] || 'Other'}
                    </span>
                    <span className="px-2 py-1 rounded-md text-[10px] font-bold bg-muted/50 border border-border text-foreground flex items-center gap-1">
                      <AlertCircle size={10} className="text-primary" /> {service.estimated_days} Days
                    </span>
                    <span className="px-2 py-1 rounded-md text-[10px] font-bold bg-muted/50 border border-border text-foreground flex items-center gap-1">
                      <ListChecks size={10} className="text-emerald-500" /> {service.steps_count} Steps
                    </span>
                 </div>
                 ...
                 <div className="grid grid-cols-4 gap-2 pt-4 border-t border-border">
                   <button 
                     ...
                     className="col-span-1 p-2 flex items-center justify-center rounded-xl bg-muted/50 text-foreground hover:bg-primary hover:text-primary-foreground transition-all"
                     title="Edit Service"
                   >
                     <Edit3 size={16} />
                   </button>
                   <button 
                     onClick={() => navigate(`/admin/jobs?service=${service.id}`)}
                     className="col-span-1 p-2 flex items-center justify-center rounded-xl bg-muted/50 text-muted-foreground hover:text-foreground transition-colors"
                     title="View Jobs"
                   >
                     <Users size={16} />
                   </button>
                   ...
                   <button 
                     ...
                     className="col-span-1 p-2 flex items-center justify-center rounded-xl bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-foreground transition-colors disabled:opacity-50"
                     title="Delete"
                   >
      ...
      {selectedServiceId && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div 
            ...
            className="bg-card border border-border w-full max-w-2xl rounded-3xl overflow-hidden relative z-10 shadow-2xl flex flex-col max-h-[85vh]"
          >
            {isDetailLoading ? (
              ...
              <>
                <div className="p-6 border-b border-border flex justify-between items-start bg-gradient-to-r from-primary/10 to-transparent">
                  <div className="flex gap-4">
                    <div className="w-14 h-14 rounded-2xl bg-muted border border-border flex items-center justify-center text-primary shadow-inner">
                      {(() => {
                    ...
                    <div>
                      <h2 className="text-2xl font-bold text-foreground leading-tight">{detailService.name_en}</h2>
                      <p className="text-sm text-muted-foreground/60 font-medium" dir="rtl">{detailService.name_ar}</p>
                    </div>
                  </div>
                  <button 
                    onClick={() => setSelectedServiceId(null)}
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-thin scrollbar-thumb-border">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="bg-muted/50 p-4 rounded-2xl border border-border">
                      ...
                      <p className="text-foreground text-xs font-bold uppercase">{detailService.steps_count} Steps</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="bg-background/50 p-4 rounded-2xl border border-border text-sm text-muted-foreground leading-relaxed">
                    ...
                    <div className="space-y-3">
                      {detailService.steps.length > 0 ? (
                        detailService.steps.map((step, idx) => (
                          <div key={step.id} className="group flex gap-4 p-4 rounded-2xl hover:bg-muted/50 border border-transparent hover:border-border transition-all">
                            ...
                            <div className="flex-1 space-y-2">
                              <div className="flex justify-between items-start">
                                <div>
                                  ...
                                  <p className="text-[11px] text-muted-foreground/60" dir="rtl">{step.name_ar}</p>
                                </div>
                              ...
                              {step.required_docs.length > 0 && (
                                <div className="flex flex-wrap gap-1.5 pt-1">
                                  {step.required_docs.map((doc, dIdx) => (
                                    <span key={dIdx} className="text-[9px] bg-muted text-muted-foreground px-2 py-0.5 rounded border border-border flex items-center gap-1">
                                      <FileCheck size={8} /> {doc}
                                    </span>
                                  ))}
                                </div>
                              )}
                ...
                <div className="p-6 border-t border-border flex gap-3">
                  ...
                  <button 
                    onClick={() => setSelectedServiceId(null)}
                    className="flex-1 py-3 bg-primary text-primary-foreground rounded-2xl text-xs font-bold transition-all shadow-lg shadow-primary/20"
                  >
                    Close Viewer
                  </button>
                </div>
