...
const InvoicePDF = ({ job, type, date = format(new Date(), 'dd MMMM yyyy') }: InvoiceProps) => {
  const isPaid = type === 'advance' ? job.advance_paid : (type === 'remaining' ? job.remaining_paid : (job.advance_paid && job.remaining_paid));
  
  const subtotal = type === 'advance' 
    ? Number(job.advance_due_amount) 
    : (type === 'remaining' ? Number(job.remaining_due_amount) : Number(job.total_fee));
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.footer}>
          ...
          <View style={{ marginTop: 20, borderTop: '2pt solid #D4AF37', paddingTop: 10 }}>
